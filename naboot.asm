...
NOP      		; 空指令以保证字节数为3
OEMName     DB "cbos0.01"; 8字节的OEMName
...
; 每个扇区的字节数 (512 1024 2048 4096)
SectorsPerCluster   DB 8
; 每个簇的扇区数 ( 1 2 4 8 16 32 64 128 )
; 两者相乘不能超过32K(簇最大大小)
ReservedSectors     DW 20h	; 从卷的第一个扇区开始的保留扇区数目；
; 该值不能为0，对于FAT12/FAT16，该值通常为1；
; 对于FAT32，典型值为32；
NumberOfFATs        DB 2
; 卷上FAT数据结构的数目，该值通常应为2
RootEntries         DW 0
; 对于FAT12/FAT16,该值表示32字节目录项的数目; 对于FAT32，该值必须为0
NumberOfSectors16   DW 0
; 该卷上的扇区总数，该字段可以为0，如果该字段
; 为0，则NumberOfSectors32不能为0；对于FAT32，
; 该字段必须为0
MediaDescriptor     DB 0xF8
; 介质类型
SectorsPerFAT16     DW 0
; 该字段标识一个FAT结构占有的扇区数（FAT12/FAT16），
; 对于FAT32卷，该字段必须为0；
SectorsPerTrack     DW 0x20
; 用于INT 0x13中断的每个磁道的扇区数
HeadsPerCylinder    DW 0x40
; 用于INT 0x13中断的每个柱面的磁头数
HiddenSectors       DD 0
; 包含该FAT卷的分区之前的隐藏扇区数
NumberOfSectors32   DD 0x0FFF7
; 该字段包含该卷上的所有扇区数目，对于FAT32，该字段
...
;
;====================================================================
SectorsPerFAT32     DD 0x01F8
; 对于FAT32，该字段包含一个FAT的大小，而SectorsPerFAT16
; 字段必须为0;
ExtFlags            DW 0
; 标志
FSVersion           DW 0
;这是文件系统的版本，高字节为主版本，低字节为次版本；
RootDirectoryStart  DD 2
;根目录的起始簇号，通常为2；
FSInfoSector        DW 1
; FSINFO结构在FAT32卷保留区域的扇区号
BackupBootSector    DW 6
; 如果该字段不为0，则表示在保留区域保存的启动记录的扇区号
; 通常为6，不推荐其他值；
Reserved1 TIMES 12  DB 0
; 保留字段，格式化程序应将该字段清0
DriveNumber         DB 0x80
; 用于INT 0x13的驱动器号，0x00为软盘，0x80为硬盘
NTReserved          DB 0
; 保留字节（用于Windows NT，即NTFS），对于FAT文件
; 系统，应始终为0
